...
def forward(initial_prob, trans_prob, obser_prob, sequence, observations, hidden_states):
    N = len(hidden_states)
    ...
    for i in range(len(sequence)):
        for j in range(N):
            if i == 0:
                ...
                val = 0
                for k in range(N):
                    val = val + alpha[k][i-1] * trans_prob[k][j] * obser_prob[j][location(observations, sequence[i])]
                alpha[j].append(val)
    return alpha
...
if __name__ == "__main__":
    ...
    sequence = [3, 1, 3]
    alpha = forward(initial_prob, trans_prob, obser_prob, sequence, observations, hidden_states)
    print("alpha : ", alpha)
